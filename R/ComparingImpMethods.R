...
CreateMissingAndObsList <- function(fact_len,num_to_remove,num_of_metabos){
  missing_list <- list()
  obs_list <- list()
  random_metabos <- c(1:num_of_metabos)
...
# factor_analysis_outputs <- FactorAnalysis(reorganized_corr_mice_med)
...
#
# imputed.df <- ImputeMissingMetabolites(sample_size,num_of_metabos,factor_mat,corr_avg,corr_array_true,
